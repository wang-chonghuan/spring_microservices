package com.wang.mgtmsv.configuration;
...
public class AMQPConfiguration {
    ...
    @Bean
    public Queue scoreQueue(@Value("${amqp.queue.score}") final String queueName) {
        return QueueBuilder.durable(queueName).build();
    }

    @Bean
    public Binding scoreBinding(final Queue scoreQueue, final TopicExchange examTopicExchange) {
        return BindingBuilder.bind(scoreQueue).to(examTopicExchange).with("score.routingkey");
    }
